...
Реакции:
- бот смотрит на emoji реакции и может ответить короткой фразой по контексту сообщения;
- позитивные реакции (`👍`, `❤️` и т.п.) бот обычно игнорирует;
- для «токсичных» реакций (например 🤡/👎/💩) бот может ответить дерзко (с cooldown, чтобы не спамить).
